...
```{r}
namq_10_gdp <- get_eurostat("namq_10_gdp", stringsAsFactors = F)
...
vidurkis<-aggregate(df$values, list(df$geo), mean)
...
```{r, fig.width=10}
ggplot (df, aes(x=time, y=values, group=geo))+
        geom_line(aes(col=geo))+
        scale_x_date(breaks=seq(min(df$time), max(df$time), by ="12 months"), date_labels="%Y")+
labs(title = "vidutinis EE, LV, LT darbo užmokesčio ir BVP santykis nuo 2000 iki naujausio turimo periodo",
subtitle="Source: Eurostat (namq_10_gdp)",
x="Laikotarpis",
y="Vid. darbo u. ir BVP santykis")
